...
class TaskService {
    ...
    async addTask(name: string, description: string, userIds: number[], columnId: number, email: string) {
    ...
    async updateTask(id: number, name: string, description: string, userIds: number[], columnId: number, email: string) {
        const boardToFind = await prismaClient.board.findFirst({
            where: {
                columns: {
                    some: { id: columnId }
        ...
        const moveDate = columnId === taskToFind.columnId ? taskToFind.moveDate : new Date()
